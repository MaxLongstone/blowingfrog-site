...
Object.assign(SPRITES, {
  ...
  hm_landlord_hide: { w: 1, h: 0.75, draw: (g, W, H) => { g.ellipse(0, H * 0.1, W * 0.36, H * 0.3).fill(0x9a8e7a); stroke(g, 3); g.circle(0, -H * 0.08, W * 0.12).fill(0xe8c9a0); stroke(g, 2); } },
  hm_umma_throw:    { w: 1, h: 1, draw: (g, W, H) => miniBoss(g, W, H, 'umma', false) },
  hm_narrator_dash: { w: 1.2, h: 1, draw: (g, W, H) => miniBoss(g, W, H, 'narrator', false) },
  hm_sackman_ghost_1: { w: 1, h: 1, draw: (g, W, H) => miniBoss(g, W, H, 'sackman', false) },
  hm_sackman_ghost_2: { w: 1, h: 1, draw: (g, W, H) => miniBoss(g, W, H, 'sackman', false) },
  ...
  env_package:  { w: 0.9, h: 0.9, draw: (g, W, H) => { g.roundRect(-W * 0.32, -H * 0.3, W * 0.64, H * 0.6, 5).fill(0xd6b483); stroke(g, 3); g.rect(-W * 0.32, -H * 0.05, W * 0.64, H * 0.1).fill(0xffffff); } },
  env_powder_puff: { w: 1.6, h: 1.2, draw: (g, W, H) => { for (const [x, y, r] of [[-0.2, 0, 0.28], [0.18, -0.1, 0.32], [0.02, 0.15, 0.26]]) g.circle(W * x, H * y, W * r).fill({ color: 0xffffff, alpha: 0.85 }); } },
  env_saucer:   { w: 2.2, h: 1, draw: (g, W, H) => { g.ellipse(0, 0, W * 0.42, H * 0.2).fill(0x8f95a0); stroke(g, 3); g.ellipse(0, -H * 0.12, W * 0.2, H * 0.14).fill(0x9fe6ff); } },
  env_cow:      { w: 1, h: 0.8, draw: (g, W, H) => { g.ellipse(0, 0, W * 0.4, H * 0.25).fill(0xf4f2ec); stroke(g, 3); g.circle(W * 0.38, -H * 0.1, W * 0.12).fill(0xf4f2ec); stroke(g, 2); } },
  env_notice:   { w: 0.8, h: 1, draw: (g, W, H) => { g.rect(-W * 0.3, -H * 0.4, W * 0.6, H * 0.8).fill(0xf4f0e6); stroke(g, 2); } },
  env_stamp:    { w: 1, h: 1, draw: (g, W, H) => { g.circle(0, 0, W * 0.36).stroke({ width: 5, color: 0xe23c2f }); } },
  env_beam_ring:{ w: 1.6, h: 0.6, draw: (g, W, H) => { g.ellipse(0, 0, W * 0.4, H * 0.3).stroke({ width: 4, color: 0x9fe6ff }); } },
  env_sack_lightbulb: { w: 0.8, h: 1, draw: (g, W, H) => { g.circle(0, -H * 0.1, W * 0.28).fill(0xffe066); stroke(g, 3); g.rect(-W * 0.1, H * 0.15, W * 0.2, H * 0.2).fill(0x8f95a0); } },
